...
class ItemDelegate(QtWidgets.QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        if option.state & QtWidgets.QStyle.State_Selected:
            ...
            pin = QtGui.QImage(AssetsMapper.PINNED_SELECTED.value)
        else:
            color.setNamedColor("#333")
            painter.setPen(color)
            pin = QtGui.QImage(AssetsMapper.PINNED.value)
...
class View:
    ...
    def populate_item_model(self):
        self.things = sorted(self.data(), key=lambda t: (t["pinned"], t["date"]), reverse=True)
